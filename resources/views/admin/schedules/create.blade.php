<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-900 leading-tight">New Schedule</h2>
    </x-slot>

    <div class="py-8 bg-sky-50/40">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="rounded-xl bg-white shadow p-6">
                <form method="POST" action="{{ route('admin.schedules.store') }}" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                    <div>
                        ...
                        <select id="owner_id" name="owner_id" class="mt-1 block w-full rounded-md border-gray-300 focus:border-sky-500 focus:ring-sky-500" required>
                            @foreach($owners as $owner)
                                <option value="{{ $owner->id }}" @selected(old('owner_id') == $owner->id)>{{ $owner->name }} ({{ ucfirst($owner->role) }})</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        <x-input-label for="file" value="Schedule File" />
                    ...
                    <div class="flex justify-end gap-2">
                        ...
                        <x-primary-button type="submit">Publish Schedule</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
